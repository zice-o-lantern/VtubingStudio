...
public:
	...
	*/
	...
	/** Enable path tracing */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Off World Live Capture Settings|Advanced", meta = (EditCondition="IsRayTracingAllowed() && IsRayTracingEnabled() && IsRayTracingEnabledForProject()"))
...
private:
	void InvertAlphaAndFinish_RenderThread(FRHICommandListImmediate& RHICmdList, UTextureRenderTarget2D* Target,  FRHITexture* InSourceTexture);
	void Finish_RenderThread(FRHICommandListImmediate& RHICmdList, UTextureRenderTarget2D* Target, FRHITexture* InSourceTexture);
